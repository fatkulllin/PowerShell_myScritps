...
for ($i=0; $i -le 49; $i++){
$searchName = $login[$i]
$AD = Get-ADUser -Filter {SamAccountName -eq $searchName} -SearchBase 'OU=Пользователи,OU=Компания,DC=org,DC=company,DC=ru'
...
"$Date`n "| Out-File D:\Profiles\ifatkullin\Desktop\files\tdmsstat501.txt
$a >> D:\Profiles\ifatkullin\Desktop\files\tdmsstat501.txt
