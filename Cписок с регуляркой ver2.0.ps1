$a = Get-Content D:\Profiles\ifatkullin\Desktop\files\stat50.txt

$a =$a -replace '\s{4}', '' #Удаляем пробелы в начале

$a = $a -replace '\s[A-z]{1,20}\s\(v3.0\)\s\([A-z]{1,10}\/[0-9]{5}\s[0-9]{1,4}\),\s[a-z]{1,5}', '' #iuser iuser Wed 7/28 10:13 Получаем такой формат
...
"$Date `n" + '<br><br>'| Out-File D:\Profiles\ifatkullin\Desktop\files\project.html
'<table><thead><tr><td>ФИО</td><td>Отдел</td><td>Имя ПК</td><td>Дата</td></tr></thead>'>> D:\Profiles\ifatkullin\Desktop\files\project.html

for ($i=0; $i -le 49; $i++){

$searchName = $login[$i]

$AD = Get-ADUser -Filter {SamAccountName -eq $searchName} -Properties Department -SearchBase 'OU=Пользователи,OU=Компания,DC=org,DC=company,DC=ru'
...
$allCell =$cellName + $cellDepartament + $cellPCName + $cellFullTime   >> D:\Profiles\ifatkullin\Desktop\files\project.html
}

'</table>' >> D:\Profiles\ifatkullin\Desktop\files\project.html
Copy-Item -Path "D:\Profiles\ifatkullin\Desktop\files\project.html" -Destination "\\avatar\d$\Portal"
